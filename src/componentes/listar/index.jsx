...
function Listar() {
  ...
  const chistesFiltrados = chistes.filter(chiste => {
    const coincideCategoria = categoriaSeleccionada === 'todos' || chiste.categoria === categoriaSeleccionada;
    const coincideBusqueda = chiste.categoria.toLowerCase().includes(busqueda.toLowerCase());
    return coincideCategoria && coincideBusqueda;
  });

  return (
    <div style={{ padding: '20px' }}>
      <h2>Chistes por categoría</h2>
      ...
      <div className="c-lista">
        {chistesFiltrados.map((chiste) => (
          <div
            className="c-lista-chiste"
            key={chiste.id}
            onClick={() => navigate(`/${chiste.categoria}`)}
